...
namespace Ztop.Todo.Model
{
    public class Code128
    {
        ...
        private Bitmap GetImage(string p_Text)
        {
            ...
            Graphics _Graphics = Graphics.FromImage(_CodeImage);
            int _LenEx = 0;
            for (var i = 0; i != _Value.Length; i++)
            {
                int _ValueNumb = Int32.Parse(_Value[i].ToString()) * (m_Magnify + 1);
                if (!((i & 1) == 0))
                {
                    _Graphics.FillRectangle(Brushes.White, new Rectangle(_LenEx, 0, _ValueNumb, (int)m_Height));
                }
                else
                {
                    _Graphics.FillRectangle(Brushes.Black, new Rectangle(_LenEx, 0, _ValueNumb, (int)m_Height));
                }
                _LenEx += _ValueNumb;
            }
            _Graphics.Dispose();
        ...
        internal Image GetCodeIamge(string p)
        {
        ...
        public Bitmap GetCodeImage(string p_Text, Encode p_Code)
        {
            ...
            for (var i = 0; i != _TextNumb.Count; i++)
            {
    ...
    public static class BarCode
    {
        ...
        public static string GetBarCodePath(string str)
        {
